...
def CNN_train(img_dir, log_dir):
    ...
    x = tf.placeholder(tf.float32, shape=[None, 1536, 3], name='input_x')
    y_ = tf.placeholder(tf.float32, shape=[None, 10], name='test_y')
    # 保存图像信息
    with tf.name_scope('input_reshape'):
        image_shaped_input = tf.reshape(x, [-1, 48, 32, 3])  # 这里是单通道哦
        tf.summary.image('input_x', image_shaped_input, 10)
    ...
    L1_relu = tf.nn.relu(L1_conv + b_conv1)  # something gone here
